...
public class AutoCollectionQueryJob extends Job {
	...
	public Result run() {
		try {
			List<CFCAOrder> cfcaOrders = cFCAOrderRepository
					.findAllByStatus(Tx1361Status.IN_PROCESSING.getStatus());
			for (CFCAOrder cfcaOrder : cfcaOrders) {
				...
				Tx1362Response response = (Tx1362Response) thirdPPService
						.invokeTx1362(request);
				if (response.getStatus() != Tx1361Status.WITHHOLDING_SUCC
						.getStatus()) {
					Transaction transaction = transactionRepository
							.findOneByReferenceAndType(cfcaOrder.getInvest()
									.getId(), Transaction.Type.CHARGE);
					transaction.setRemark(Transaction.Status.RECHARGE_SUCC);
					User user = cfcaOrder.getInvest().getUser();
					UserAccount cashAccount = userAccountRepository
							.findByUserAndType(user, UserAccount.Type.CASH);
					cashAccount.setBalance(cashAccount.getBalance().add(
							cfcaOrder.getAmount()));
					userAccountRepository.save(cashAccount);
					transactionService.freeze(Transaction.Type.FREEZE,
							user.getId(), cfcaOrder.getInvest().getAmount(),
							cfcaOrder.getInvest().getLoan().getId(), "投标冻结");
					cfcaOrder.setStatus(Tx1361Status.WITHHOLDING_SUCC
							.getStatus());
					Invest invest = investRepository.findOne(cfcaOrder
							.getInvest().getId());
					invest.setStatus(Invest.Status.FREEZE);
					investRepository.save(invest);
					cFCAOrderRepository.save(cfcaOrder);
					transactionRepository.save(transaction);
				} else if (response.getStatus() == Tx1361Status.WITHHOLDING_SUCC
						.getStatus()) {
					Transaction transaction = transactionRepository
							.findOneByReferenceAndType(cfcaOrder.getInvest()
									.getId(), Transaction.Type.CHARGE);
					...
					Loan loan = loanRepository.findOne(cfcaOrder.getInvest()
							.getLoan().getId());
					loan.setProceeds(loan.getAmount().subtract(
							cfcaOrder.getInvest().getAmount()));
					cfcaOrder.setStatus(Tx1361Status.WITHHOLDING_FAIL
							.getStatus());
